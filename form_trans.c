...
//	  printf("%s%f%f%f\n",a,eig1,eig2,eig3);
  ...
  //  eigenval2_x = (float)x*expconst;   
  //   i = random_number(&x);
  //  eigenval2_y = (float)x*expconst;
  //   i = random_number(&x);
  //  eigenval2_z = (float)x*expconst;
...
//      return 0;
      ...
      scalekey =  (-1)*expconst/(scansteps*D_A2);
//      scalekey = 0.0011;
...
/*      fp = fopen("t.ndx","w");
      ...
      if(don > 0 && hyd > 0 && acc > 0) fprintf(fp,"%d\n%d\n%d\n",don,hyd,acc);
